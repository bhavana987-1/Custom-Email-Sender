...
def browse():
    ...
    if path == '':
        ...
        try:
            # Check file extension and use the appropriate engine
            if path.endswith('.xlsx'):
                data = pandas.read_excel(path, engine='openpyxl')
            elif path.endswith('.xls'):
                data = pandas.read_excel(path, engine='xlrd')
            else:
                messagebox.showerror('Error', 'Unsupported file format. Please select a valid Excel file.')
                return

            # Check for the 'Email' column
            if 'Email' in data.columns:
                emails = list(data['Email'])
                final_emails = [email for email in emails if pandas.notnull(email)]
                ...
                messagebox.showerror('Error', 'The selected Excel file does not contain an "Email" column')
...
def setting():
    ...
    Label(root1,text='Credential Settings',image=logoImage,compound=LEFT,font=('Helvetica 20',30,'bold'),
          fg='black',bg='white').grid(padx=60)

    fromLabelFrame = LabelFrame(root1, text='From (Email Address)', font=('times new roman', 16, 'bold'), bd=5, fg='black',
                              bg='white')
...
titleLabel=Label(titleFrame,text="   Email",image=logoImage,compound=LEFT,font=('Helvetica 20',28,'bold'),bg='white',fg='black')
titleLabel.grid(row=0,column=0)
settingImage=PhotoImage(file='setting.png')
Button(titleFrame,image=settingImage,bd=0,bg='white',cursor='hand2',activebackground='white',command=setting).grid(row=0,column=1,padx=20)

chooseFrame=Frame(root,bg='white')
chooseFrame.grid(row=1,column=0,pady=10)

choice=StringVar()

singleRadioButton=Radiobutton(chooseFrame,text='Single',font=('Helvetica 20',25,'bold')
                              ,variable=choice,value='single',bg='white',activebackground='white',command=button_check)
singleRadioButton.grid(row=0,column=0,padx=20)

multipleRadioButton=Radiobutton(chooseFrame,text='Multiple',font=('Helvetica 20',25,'bold')
                              ,variable=choice,value='multiple',bg='white',activebackground='white',command=button_check)
multipleRadioButton.grid(row=0,column=1,padx=20)

choice.set('single')

toLabelFrame=LabelFrame(root,text='To (Email Address)',font=('Helvetica 20',16,'bold'),bd=5,fg='black',bg='white')
toLabelFrame.grid(row=2,column=0,padx=100)

toEntryField=Entry(toLabelFrame,font=('Helvetica 20',18,'bold'),width=30)
...
browseButton=Button(toLabelFrame,text=' Browse',image=browseImage,compound=LEFT,font=('arial',12,'bold'),
       cursor='hand2',bd=0,bg='white',activebackground='white',state=DISABLED,command=browse)
browseButton.grid(row=0,column=1,padx=20)

subjectLabelFrame=LabelFrame(root,text='Subject',font=('Helvetica 20',16,'bold'),bd=5,fg='black',bg='white')
subjectLabelFrame.grid(row=3,column=0,pady=10)

subjectEntryField=Entry(subjectLabelFrame,font=('Helvetica 20',18,'bold'),width=30)
subjectEntryField.grid(row=0,column=0)

emailLabelFrame=LabelFrame(root,text='Compose Email',font=('Helvetica 20',16,'bold'),bd=5,fg='black',bg='white')
...
Button(emailLabelFrame,text='Speak',image=micImage,compound=LEFT,font=('arial',12,'bold'),
       cursor='hand2',bd=0,bg='white',activebackground='white',command=speak).grid(row=0,column=0)
...
Button(emailLabelFrame,text='Attachment',image=attachImage,compound=LEFT,font=('arial',12,'bold'),
       cursor='hand2',bd=0,bg='white',activebackground='white',command=attachment).grid(row=0,column=1)
...
Button(root,image=sendImage,bd=0,bg='white',cursor='hand2',activebackground='white',command=send_email).place(x=490,y=540)
clear
clearImage=PhotoImage(file='clear.png')
Button(root,image=clearImage,bd=0,bg='white',cursor='hand2',activebackground='white',command=clear).place(x=590,y=550)

exitImage=PhotoImage(file='exit.png')
Button(root,image=exitImage,bd=0,bg='white',cursor='hand2',activebackground='white',command=iexit).place(x=690,y=550)


totalLabel=Label(root,font=('times new roman',18,'bold'),bg='white',fg='black')
totalLabel.place(x=10,y=560)

sentLabel=Label(root,font=('times new roman',18,'bold'),bg='white',fg='black')
sentLabel.place(x=100,y=560)

leftLabel=Label(root,font=('times new roman',18,'bold'),bg='white',fg='black')
leftLabel.place(x=190,y=560)

failedLabel=Label(root,font=('times new roman',18,'bold'),bg='white',fg='black')
